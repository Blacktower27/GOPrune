...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # === 在 EncoderLayer 直接保存 key 和 query ===
        self.metric_key = self.self_attn.k_proj(hidden_states)  # [B, N, D]
        self.metric_query = self.self_attn.q_proj(hidden_states)  # [B, N, D]
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        processed_layer = self.extract_semantic_tokens_from_vision_outputs(encoder_outputs.hidden_states,encoder_outputs.attentions)
        hidden_states = list(encoder_outputs.hidden_states)
        hidden_states[-2] = processed_layer
        new_hidden_states = tuple(hidden_states)
        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state,
            pooler_output=pooled_output,
    ...
    @staticmethod
    def extract_semantic_tokens_from_vision_outputs(hidden_states, attentions,select_layer_start=6, select_layer_end=9):
        """
...
class CLIPVisionModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig):
        super().__init__(config)
        config.output_attentions = True  # change default value to True
